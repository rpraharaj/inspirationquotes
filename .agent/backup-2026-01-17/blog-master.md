...
> **📚 Standards Reference:** See `/blog-quality-standards` for all quality requirements. This is the single source of truth.
...
6. **Use speed optimizations** - Check caches and indexes before searching

### ⚡ Pre-Execution: Load Support Files

Before starting, load these for faster execution:

```
.agent/ai-models-current.json          ← For AI-related topics
blogpost-content-plan/internal-link-index.json  ← For internal linking
```

**Required sub-workflows:**
- `/blog-research` - Phase 1 (uses AI model cache + parallel execution)
- `/blog-outline` - Phase 2
- `/blog-writer` - Phase 3 (uses quick checks + link index)
- `/blog-reviewer` - Phase 4
- `/featured-image` - Phase 4.5 (black & white OG images with watermark)
- `/blog-validator` - Phase 5
- `/blog-writing-sop` - Phase 6 (publishing)
